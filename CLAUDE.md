...
- **Fetch prices**: `python3 scripts/fetch_prices.py` — reads `data/groups.json`, queries Yahoo Finance, writes `data/latest.json`. Note: local networks with allowlists may not reach `query1.finance.yahoo.com`; it works in GitHub Actions.
- **Preview locally**: serve over HTTP (e.g. `python3 -m http.server`), since `index.html` fetches `data/*.json` and that fails over `file://`.
